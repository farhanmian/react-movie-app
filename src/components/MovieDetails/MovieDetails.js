...
function MovieDetails(props) {
  ...
  const noImage =
    "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS2O_mIRhxseMzEZB_rvzjC92FQmBsVxY3wIrFnGM-Oh6azjRI_InZDtPmihRINHJGVVO4&usqp=CAU";
    ...
    <div className={classes.MovieDetails}>
      <div className={classes.InnerContainer}>
        <h1 className={classes.Heading}>
          {props.details.title} ({props.details.release_date})
        </h1>
        ...
        <div className={classes.Details}>
          <span className={classes.ImageContainer}>
            <img
              src={
                props.details.image
                  ? `${imgBaseUrl}/${props.details.image}`
                  : movieNoImg
              }
              alt="movieImg"
              style={{ filter: props.loading ? "blur(3px)" : "blur(0)" }}
            />
          </span>
            ...
            <span className={classes.Genres}>
              ...
              <h3>
                {props.details.genres &&
                  props.details.genres.map(
                    (e, i) =>
                      `${e.name} ${
                        i < props.details.genres.length - 1 ? "," : ""
                      } `
                  )}
              </h3>
            </span>
            ...
            <div className={classes.Credits}>
              {props.movieCastDetail &&
                props.movieCastDetail.map((cast, i) => {
                  const castName =
                    cast.name.trim().length > 13
                      ? `${cast.name.slice(0, 13)}...`
                      : cast.name;
                  return (
                    <span id={i} key={cast.id}>
                      <img
                        src={
                          cast.profile_path
                        ...
                        alt="action img"
                        style={{
                          filter: props.loading ? "blur(3px)" : "blur(0)",
                        }}
                      />
                      <p>{castName || cast.original_name}</p>
                    </span>
                  );
                })}
            </div>
        ...
        <div className={classes.Similar}>
          <h1 className={`${classes.Heading} ${classes.HeadingSimilar}`}>
            Similar {props.type === "tv" ? "Tv Series" : "Movies"}
          </h1>
          <MovieList
            className={classes.SimilarResults}
            movies={props.similar}
            type={props.type === "tv" ? "Tv Series" : "Movie"}
          />
        </div>
      </div>
    </div>
